...
from aimacode.search import (InstrumentedProblem, astar_search,
                             breadth_first_search, breadth_first_tree_search,
                             depth_first_graph_search, depth_limited_search,
                             greedy_best_first_graph_search,
                             recursive_best_first_search, uniform_cost_search)
...
PROBLEMS = [["Air Cargo Problem 1", air_cargo_p1], ["Air Cargo Problem 2", air_cargo_p2],
...
def run_search(pname, sname, hname, problem, search_function, parameter=None):
    ...
    delta = end - start
    plan = ["{}{}".format(a.name, a.args) for a in node.solution()]
...
def manual():
    ...
    print("\n  python {} -p {} -s {}\n".format(__file__, " ".join(p_choices), " ".join(s_choices)))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        '--problems',
        nargs="+",
        choices=range(1, len(PROBLEMS) + 1),
        type=int,
        metavar='',
        help="Specify the indices of the problems to solve as a list of space separated values. Choose from: {!s}".
